...
export async function DELETE(
  ...
  try {
    ...
    const file = await (prisma as any).clubActivityPlanFile.findFirst({
      where: {
        id: fileId,
        clubId,
        school,
    ...
    await (prisma as any).clubActivityPlanFile.delete({
      where: { id: file.id },
    });

    return NextResponse.json({ message: "파일이 삭제되었습니다." });
  } catch (error) {
    console.error("Club activity plan delete error:", error);
    return NextResponse.json(
