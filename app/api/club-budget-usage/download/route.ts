...
export async function GET(request: NextRequest) {
  try {
    ...
    if (targetClubIds.length === 0) {
      return NextResponse.json(
        { error: "다운로드할 동아리가 없습니다." },
        { status: 404 }
      );
    }

    const files = await (prisma as any).clubBudgetUsagePlanFile.findMany({
      where: {
        school,
    ...
    const clubNameById = new Map<string, string>(
      targetClubs.map((club: any) => [String(club.id), String(club.clubName || "")])
    );

    const zipName =
      clubType === "creative"
        ? "창체 동아리_예산_사용_계획서.zip"
        : "자율 동아리_예산_사용_계획서.zip";
    ...
    void (async () => {
      try {
        ...
        for (const f of files) {
          const url = String((f as any).filePath || "");
          if (!url) continue;

          const clubName = sanitizeFileName(
            clubNameById.get(String((f as any).clubId || "")) || "동아리"
          );
          const originalName = sanitizeFileName(String((f as any).originalFileName || "file"));
        ...
        console.error("Club budget usage ZIP stream error:", err);
    ...
    console.error("Club budget usage ZIP download error:", error);
    return NextResponse.json(
