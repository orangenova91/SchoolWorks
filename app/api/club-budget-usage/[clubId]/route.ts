...
export async function GET(
  ...
  try {
    ...
    console.error("Club budget usage plan list error:", error);
    return NextResponse.json(
      { error: "예산 사용 계획서 목록을 불러오는 데 실패했습니다." },
      { status: 500 }
    );
  }
}
...
export async function POST(
  ...
  try {
    ...
    const filename = `club-budget-usage/${safeSchool}/${club.id}/${timestamp}-${randomStr}.${finalExt}`;
    ...
    await (prisma as any).clubBudgetUsagePlanFile.deleteMany({
      where: { school, clubId: club.id },
    });

    const record = await (prisma as any).clubBudgetUsagePlanFile.create({
      data: {
        clubId: club.id,
        school,
        filePath: blob.url,
    ...
    console.error("Club budget usage plan upload error:", error);
    return NextResponse.json(
      { error: "예산 사용 계획서 업로드 중 오류가 발생했습니다." },
      { status: 500 }
    );
  }
}
