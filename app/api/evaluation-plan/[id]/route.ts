...
export async function DELETE(
  ...
  try {
    ...
    const file = await (prisma as any).evaluationPlanFile.findUnique({
      where: { id },
    });
    ...
    await (prisma as any).evaluationPlanFile.delete({
      where: { id },
    });

    return NextResponse.json({ message: "파일이 삭제되었습니다." });
  } catch (error) {
    console.error("Evaluation plan file delete error:", error);
