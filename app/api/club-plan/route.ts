...
export async function GET() {
  try {
    ...
    const files = await (prisma as any).clubPlanFile.findMany({
      where: { school },
    ...
    console.error("Club plan list error:", error);
    return NextResponse.json(
      { error: "동아리 계획서 목록을 불러오는 데 실패했습니다." },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const filename = `club-plan/${school.replace(
      /[^a-zA-Z0-9가-힣_-]/g,
      "_"
    ...
    const record = await (prisma as any).clubPlanFile.create({
      data: {
        school,
        filePath: blob.url,
    ...
    console.error("Club plan upload error:", error);
    return NextResponse.json(
