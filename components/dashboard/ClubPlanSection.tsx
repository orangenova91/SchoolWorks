...
export default function ClubPlanSection() {
  const [files, setFiles] = useState<ClubPlanFile[]>([]);
  ...
  const fetchFiles = async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await fetch("/api/club-plan");
      if (!res.ok) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      setDeletingId(id);
      const res = await fetch(`/api/club-plan/${id}`, {
        method: "DELETE",
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
  ...
  const handleDownloadZip = () => {
    ...
    link.href = "/api/club-plan/download";
  ...
  if (error) {
    return (
      <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        <h2 className="text-lg font-semibold text-gray-900 mb-2">동아리 계획서</h2>
    ...
    <div className="space-y-4">
      <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 className="text-lg font-semibold text-gray-900 mb-1">동아리 계획서</h2>
