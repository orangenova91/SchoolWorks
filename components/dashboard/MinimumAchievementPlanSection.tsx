...
type MinimumAchievementPlanFile = {
  id: string;
  grade: string;
  semester?: string;
  filePath: string;
...
export default function MinimumAchievementPlanSection() {
  const [files, setFiles] = useState<MinimumAchievementPlanFile[]>([]);
  ...
  const fetchFiles = async () => {
    try {
      ...
      const res = await fetch(
        `/api/minimum-achievement-plan?semester=${encodeURIComponent(semester)}`
      );
      if (!res.ok) {
  ...
  const uploadFile = async (grade: string, file: File) => {
    try {
      ...
      const res = await fetch("/api/minimum-achievement-plan", {
        method: "POST",
        body: formData,
      });
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      setDeletingId(id);
      const res = await fetch(`/api/minimum-achievement-plan/${id}`, {
        method: "DELETE",
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
    ...
    <div className="space-y-4">
      <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <p className="text-sm text-gray-600">
          학년별로 최소성취수준보장 계획서 파일을 업로드하고 관리할 수 있습니다.
        </p>
      </div>
                    ...
                    getFilesByGrade(value).map((f) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <a
                            href={`/api/minimum-achievement-plan/file/${f.id}`}
                            target="_blank"
